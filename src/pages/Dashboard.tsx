    ...
    <div className="grid lg:grid-cols-5 gap-6">
      <div className="lg:col-span-3 rounded-3xl bg-card border border-border/40 p-6 lg:p-8 shadow-sm flex flex-col items-center space-y-6">
        ...
        <div className="flex flex-wrap gap-3 justify-center">
          ...
          <Button onClick={() => { const el = document.getElementById("qr-printable"); if (!el) return; const w = window.open("", "_blank"); if (!w) return; w.document.write(`<!DOCTYPE html><html><head><title>QR - ${business.name}</title><style>body{display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0;font-family:system-ui,sans-serif;}</style></head><body>${el.outerHTML}</body></html>`); w.document.close(); w.focus(); w.print(); }} variant="outline" size="sm" className="rounded-xl gap-2 text-xs h-10 px-4"><Printer className="w-4 h-4" /> Imprimer</Button>
        </div>
      </div>
